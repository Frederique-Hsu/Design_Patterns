...
这种模式涉及一个单一的类，该类负责加入独立的或不兼容的接口功能。

举一个真实的例子： 读卡器是作为内存卡和笔记本电脑之间的适配器。你讲内存卡插入读卡器，再将读卡器插入笔记本电脑，这样就可以让笔记本电脑来读取内存卡了。
...
- 希望创建一个可复用的类， 与多个不相关的的类（包括未来可能引入的类）一起工作，这些类可能没有统一的接口。
...
![Adapter Pattern](adapter_pattern.png "Adapter Pattern")